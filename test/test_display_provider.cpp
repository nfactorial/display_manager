...
TEST( DisplayProvider, Construction ) {
    ngen::renderer::DisplayProvider provider;
...
TEST(DisplayProvider, CreatePort) {
    ...
    std::array<ngen::renderer::DisplayPort *, ngen::renderer::kDisplayPortCapacity> ports;

    for (size_t loop = 0; loop < ngen::renderer::kDisplayPortCapacity; ++loop) {
        ports[loop] = provider.createDisplayPort(kPipelineName);
        EXPECT_NE(nullptr, ports[loop]);
    }

    EXPECT_EQ(ngen::renderer::kDisplayPortCapacity, provider.getDisplayPortCount());
